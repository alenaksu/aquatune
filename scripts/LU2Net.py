...
class AxialDW(nn.Module):
    def __init__(self, dim, mixer_kernel, dilation = 1):
        super().__init__()
        h, w = mixer_kernel
        # Explicit 'same' padding so ONNX Runtime handles dilated convs correctly:
        #   pad = dilation * (kernel - 1) / 2  (integer, since kernels are odd)
        pad_h = dilation * (h - 1) // 2
        pad_w = dilation * (w - 1) // 2
